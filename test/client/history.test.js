...
const KEY = 'icy.history.v1';

describe('recording', () => {
  it('keeps both sides of the conversation in the order they were said', () => {
    const { history } = harness();
    history.begin({ model: 'grok-voice-latest', voice: 'atlas' });
    history.append({ role: 'user', content: 'what are you, exactly?' });
    history.append({ role: 'assistant', content: 'Frozen water, briefly. So are you, given time.' });
    ...
    assert.deepEqual(conversation.messages.map((m) => [m.role, m.content]), [
      ...
      ['assistant', 'Frozen water, briefly. So are you, given time.'],
    ]);
    assert.equal(conversation.model, 'grok-voice-latest');
    assert.equal(conversation.voice, 'atlas');
  });
...
describe('picking one back up', () => {
  function stored() {
    ...
    history.begin({ voice: 'atlas' });
    history.append({ role: 'user', content: 'what are you, exactly?' });
    history.append({ role: 'assistant', content: 'Frozen water, briefly.' });
  ...
  it('hands back what was said in the one it reopens', () => {
    ...
    assert.deepEqual(earlier.messages.map((m) => m.content), ['what are you, exactly?', 'Frozen water, briefly.']);
  });
