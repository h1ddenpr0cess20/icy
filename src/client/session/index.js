 ...
 * vocabulary. The page wires those events to Icy once; Icy
 ...
 *   'level'        0..1 sustained amplitude — mic while listening, Icy while speaking
 ...
 * page's wiring block and Icy do not change.
 */
...
const MIC_CONSTRAINTS = {
  audio: {
    // Icy's voice comes out of the speakers and straight back into the mic.
...
export function createVoiceSession({ model, voice } = {}) {
  ...
  /* One loop for both jobs that have to happen every frame: metering, and
     ...
     that knows when Icy has actually stopped talking — `response.done` fires
  ...
  return {
     ...
     *  stop; cancelling one without the other leaves Icy talking into a turn
