 ...
 * override — those are the two ways a page could rewrite who Icy is.
 */
...
export function sanitize(event) {
  ...
  if (event.type === 'conversation.item.create') {
    // Only the person's own turns. `force_message` would put words in Icy's
...
export function createRealtimeProxy(config) {
  ...
  wss.on('connection', (client, req) => {
    ...
    /* Frames the page sends before xAI answers the handshake. The mic can open
       ...
       makes Icy miss the start of the first thing anyone says to him. */
    let pending = [];
