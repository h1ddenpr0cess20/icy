...
under `icy.memory.v1`. Neither is uploaded — the proxy holds no copy of either. The tool switches are a third,
`icy.tools.v1`, holding the names that are switched *off* — so a tool nobody
